...
public class Room implements Serializable {
    ...
    public String getObjectString() {
        //initialize string
        String holder = "";
        //add each object name to the string
        for (int i = 0; i < objectsInRoom.size(); i++) {
            holder += objectsInRoom.get(i).getDescription();
            holder += ", ";
        }
        //remove the last comma
        holder = holder.replaceAll(", $","");
        //return the string with all the object names
        return holder;
    }
    ...
    public String getCommands() {
        //initialize string
        String holder = "";
        //get all passages for room from motion table
        List<Passage> curr = motionTable.getDirection();
        List<String> dir = new ArrayList<>();
        for (int i = 0; i < curr.size(); i++){
            String x = curr.get(i).getDirection();
            //no direction should show twice
            if (!dir.contains(x)){
                dir.add(x);
                holder += x;
                holder += ", ";
